...
public class FlowTagLayout extends ViewGroup {
     ...
     * FlowLayout support multi-select
     */
    public static final int FLOW_TAG_FIRSTVIEW_CHECKED_SINGLE_OTHER_CHECKED_MULTI = 3;
    /**
    ...
    private void reloadData() {
        ...
        for (int i = 0; i < mAdapter.getCount(); i++) {
            ...
            if (mAdapter instanceof OnInitSelectedPosition) {
                ...
                } else if (mTagCheckMode == FLOW_TAG_FIRSTVIEW_CHECKED_SINGLE_OTHER_CHECKED_MULTI) {
                    //如果第一个被选中，则其他选项不能被选中
                    if (i == 0 && isSelected && !isSetted) {
                        mCheckedTagArray.put(i, true);
                        childView.setSelected(true);
                        isSetted = true;
                    } else if (i != 0 && isSelected && !isSetted) {
                        //第一个不能被选中
                        if (mCheckedTagArray.get(0)) {
            ...
            childView.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (mTagCheckMode == FLOW_TAG_CHECKED_NONE) {
                        ...
                        //选中的是第一个，第一个已经被选中
                        if (j == 0 && mCheckedTagArray.get(0)) {
                            //被选中，则不做任何处理
                            return;
                        }//选中的是第一个，第一个未被选中
                        else if (j == 0 && !mCheckedTagArray.get(0)) {
                            //第一个被选中，其他childView不被选中
                            mCheckedTagArray.put(j, true);
                            childView.setSelected(true);
